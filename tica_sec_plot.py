...
data_SB = np.loadtxt('protein_ligand_dist.dat')
...
d=ax[1].scatter(cc_x, cc_y, c=clustering.clustercenters[:,6],cmap='gnuplot', marker='o', vmin=104.0,vmax=380.0)
#ax[1].set_xlabel('SB')
plt.colorbar(d, ax=ax[1])
plt.tight_layout()
plt.show()
plt.savefig('ss_colored_Q_frac.png',dpi=300)

#fig, ax = plt.subplots(1, 2, sharex=True, sharey=True)
#cc_x = clustering.clustercenters[:,0]
#cc_y = clustering.clustercenters[:,1]
#pyemma.plots.plot_free_energy(np.vstack(tica_concatenated)[:,0], np.vstack(tica_concatenated)[:,1], ax=ax[0], cbar=True)
#ax[1].scatter(cc_x,cc_y, c=clustering.clustercenters[:,3],cmap='gnuplot', marker='o', vmin=0,vmax=0.6)
#ax[1].set_xlabel('Helix')
#ax[2].scatter(cc_x, cc_y, c=clustering.clustercenters[:,4],cmap='gnuplot', marker='o', vmin=0,vmax=0.6)
#ax[2].set_xlabel('Beta')
#ax[3].scatter(cc_x, cc_y, c=clustering.clustercenters[:,5],cmap='gnuplot', marker='o', vmin=0,vmax=0.6)
#ax[3].set_xlabel('Other')
#d=ax[1].scatter(cc_x, cc_y, c=clustering.clustercenters[:,5],cmap='gnuplot', marker='o', vmin=0,vmax=1.0)
#ax[1].set_xlabel('SB')
#plt.colorbar(d, ax=ax[1])
#plt.tight_layout()
#plt.show()
#plt.savefig('ss_colored_dssp.png',dpi=300)
