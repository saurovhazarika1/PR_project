...
traj2 = md.load(dcd, top=psf)

feat = coor.featurizer(top)
feat.add_minrmsd_to_ref(traj2)

reader = pyemma.coordinates.source(trajs, features=feat) 

A = feat.topology.select("resid 229 and name CA")

B = feat.topology.select("resid 240 and name CA")
...
print('MFPT 1 -> 2: ({:6.1f} +- {:5.1f}) ns'.format(
    0.05*M.sample_mean('mfpt', A, B), 0.05*M.sample_std('mfpt', A, B)))

print('MFPT 1 -> 3: ({:.1f} +- {:5.1f}) ns'.format(
    0.05*M.sample_mean('mfpt', A, C), 0.05*M.sample_std('mfpt', A, C)))

print('MFPT 2 -> 1: ({:.1f} +- {:5.1f}) ns'.format(
    0.05*M.sample_mean('mfpt', B, A), 0.05*M.sample_std('mfpt', B, A)))

print('MFPT 2 -> 3: ({:.1f} +- {:5.1f}) ns'.format(
    0.05*M.sample_mean('mfpt', B, C), 0.05*M.sample_std('mfpt', B, C)))

print('MFPT 3 -> 1: ({:.1f} +- {:5.1f}) ns'.format(
    0.05*M.sample_mean('mfpt', C, A), 0.05*M.sample_std('mfpt', C, A)))

print('MFPT 3 -> 2: ({:.1f} +- {:5.1f}) ns'.format(
    0.05*M.sample_mean('mfpt', C, B), 0.05*M.sample_std('mfpt', C, B)))
